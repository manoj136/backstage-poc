...
Once the administrator has finished configuring the user, he will activate the user using the "confirm action" on the front-end application.
This will start a multistep asyncronous process that will:

- Create the user on the IDP.
- Change the status of the user on the omega database to Active
- Send a mail to the user notifying that he can now login on the application

> Note: 
The following diagram ilustrates the flow of a single user activation. Triggers will process the message on the queues as they are received.
## Diagram

```plantuml format="svg_inline"
@startuml
title User profile confirmation 
actor Admin as Adm #yellow
...
actor User as U 
Adm -> F: **(1)** Confirm user
F -> UPA: **(2)** PATCH UserProfile
activate UPA
UPA -> ST: **(3)** POST CreateOrSelectGlobalUser
activate ST
ST --> UPA: LegacyId
deactivate ST
UPA -> DB: Stores changes
...
> 1. The **Admin** selects the user to confirm and use the confirm-user action
> 2. The **Frontend** send the user confirmation request to the UserProfileApi
> 3. The **Front-office.UserProfileApi** request the user's legacyID from the **Stock-Tools api**, stores the changes in the 
database, and set the status to active.
